...
export function ExportButton({ transactions, month, year }: ExportButtonProps) {
  ...
  const handleDownloadExcel = () => {
    ...
    data.push({
      No: '', Tanggal: '', Kategori: '', Uraian: 'TOTAL PERIODE INI', 
      Pemasukan: totalIncome, Pengeluaran: totalExpense
    } as any)
  ...
  return (
    <div className="relative h-full">
      <button 
        ...
        className="bg-white border border-slate-200 text-slate-700 px-4 py-2.5 rounded-lg hover:bg-slate-50 flex items-center gap-2 font-medium shadow-sm transition-all"
      >
        <Printer size={18} />
        <span>Export</span>
        <ChevronDown size={16} />
